...
const PORTFOLIO_URL =
  process.env.NEXT_PUBLIC_SERVER_URL ?? "https://photosby.joeldettinger.de";
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111">
        ...
        <p>Click the link below to view your gallery and select your favourite images.</p>
        ...
        <p style="color:#666;font-size:13px">Or copy this link: <a href="${approvalUrl}">${approvalUrl}</a></p>
      </div>
    `,
  });
}
...
// --------------------------------------------------------------------------
// Selections submitted — admin notification
// --------------------------------------------------------------------------
...
export async function sendSelectionsNotification({
  ...
  const adminUrl = `${process.env.NEXT_PUBLIC_SERVER_URL ?? PORTFOLIO_URL}/admin/collections/albums`;
  await transporter.sendMail({
    from: `"Joel Dettinger Photography" <${MAIL_FROM}>`,
    to: ADMIN_TO,
...
export async function sendNewReviewNotification({
  ...
  const adminUrl = `${process.env.NEXT_PUBLIC_SERVER_URL ?? PORTFOLIO_URL}/admin/collections/reviews`;
  await transporter.sendMail({
    from: `"Joel Dettinger Photography" <${MAIL_FROM}>`,
    to: ADMIN_TO,
